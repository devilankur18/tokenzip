...
import { 
  Activity, FileCode, Zap, Cpu, Settings, 
  Layers, Search, GitMerge, Info, Play, 
  Check, Copy, RefreshCw, 
  BookOpen, AlertTriangle, Eye, Server, Compass
} from 'lucide-react';
import { Surreal } from 'surrealdb';

import InteractiveAsciiTree from './InteractiveAsciiTree';
import { ToolCodeSnapshotInputs, ToolCodeSnapshotROI } from './tools/ToolCodeSnapshot';
import { ToolCodeReadInputs, ToolCodeReadROI, ToolCodeReadOutput } from './tools/ToolCodeRead';
import { ToolCodeSearchInputs, ToolCodeSearchROI, ToolCodeSearchOutput } from './tools/ToolCodeSearch';
import { ToolCodeTraceFlowInputs, ToolCodeTraceFlowROI, ToolCodeTraceFlowOutput } from './tools/ToolCodeTraceFlow';
import { ToolCodeInsightInputs, ToolCodeInsightSaveForm, ToolCodeInsightROI, ToolCodeInsightOutput } from './tools/ToolCodeInsight';
import ToolLegacyCompare from './tools/ToolLegacyCompare';
...
const Playground: React.FC<PlaygroundProps> = ({ db, repoInfo, initialFile, onFileChange, appMode, setAppMode }) => {
  ...
  const renderComparativeROI = () => {
    switch (activeTool) {
      case 'code_snapshot':
        return <ToolCodeSnapshotROI />;
      case 'code_read':
        return <ToolCodeReadROI codeReadCompare={codeReadCompare} selectedFile={selectedFile} />;
      case 'code_search':
        return <ToolCodeSearchROI />;
      case 'code_trace_flow':
        return <ToolCodeTraceFlowROI />;
      case 'code_insight':
        return <ToolCodeInsightROI />;
      default:
        return null;
    }
  };
        ...
        <div className="playground-main" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', padding: '24px', background: 'radial-gradient(circle at 70% 10%, rgba(99,102,241,0.03), transparent 50%)', position: 'relative' }}>
          ...
          <div className="params-panel" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', padding: '20px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
              ...
              {activeTool === 'code_insight' && (
                <ToolCodeInsightInputs 
                  ...
                  insightNoteCategory={insightNoteCategory}
                  setInsightNoteCategory={setInsightNoteCategory}
                />
              )}
            ...
            {activeTool === 'code_insight' && insightAction === 'save' && (
              <ToolCodeInsightSaveForm 
                insightNoteTitle={insightNoteTitle}
                setInsightNoteTitle={setInsightNoteTitle}
                insightNoteSummary={insightNoteSummary}
                setInsightNoteSummary={setInsightNoteSummary}
                insightNoteScope={insightNoteScope}
                setInsightNoteScope={setInsightNoteScope}
              />
            )}
          </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'row', width: '100%', gap: '16px', alignItems: 'stretch' }}>
                      ...
                      {activeTool === 'code_snapshot' && snapshotFormat === 'tree' ? (
                        ...
                        <ToolCodeReadOutput 
                          ...
                          getCleanResultText={getCleanResultText}
                          codeReadCompare={codeReadCompare}
                          batchCompareData={batchCompareData}
                          batchRawContents={batchRawContents}
                          readMode={readMode}
                          rawFileContent={rawFileContent}
                        />
                      ) : activeTool === 'code_search' ? (
                        <ToolCodeSearchOutput 
                          toolResult={toolResult}
                          onSelectMatch={(filePath) => {
                        ...
                        <ToolCodeTraceFlowOutput 
                          toolResult={toolResult}
                          onSelectSymbol={(symbolName) => {
                            setTraceTarget(symbolName);
                            executeToolCall('code_trace_flow', {
                              target: symbolName,
                              direction: traceDirection
                            });
                      ...
                      <ToolCodeInsightOutput 
                        toolResult={toolResult}
                        getParsedInstructions={getParsedInstructions}
